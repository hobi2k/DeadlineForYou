...
4. 사용자가 끝날 때까지 보고하지 않으면 세션을 `0`으로 자동 마감
5. 자동 마감 뒤에는 LLM이 새 `/timer <분>` 을 다시 시작하라고 재촉
...
- 타이머가 끝난 뒤 너무 늦으면 그 세션은 자동으로 `0` 처리되어 닫힐 수 있다.
...
일반 텔레그램 대화에서는 번역 여부를 LLM이 판단하고, 직접 명령 `/translate`는 즉시 번역 모델을 호출한다.
...
일반 텔레그램 대화에서는 이미지 요청 여부를 LLM이 판단하고, 직접 명령 `/image`는 즉시 이미지 모델을 호출한다.
